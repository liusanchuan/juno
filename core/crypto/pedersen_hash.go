...
func Pedersen(a *felt.Felt, b *felt.Felt) *felt.Felt {
	aBytes := a.Bytes()
	bBytes := b.Bytes()
	aElement := new(fp.Element).SetBytes(aBytes[:])
	bElement := new(fp.Element).SetBytes(bBytes[:])
	result := new(starkcurve.G1Jac).Set(shiftPoint)
	result.AddAssign(processElement(aElement, p0, p1))
	result.AddAssign(processElement(bElement, p2, p3))
	resultBytes := new(starkcurve.G1Affine).FromJacobian(result).X.Bytes()
	return new(felt.Felt).SetBytes(resultBytes[:])
}
...
func processElement(a *fp.Element, p1 *starkcurve.G1Jac, p2 *starkcurve.G1Jac) *starkcurve.G1Jac {
	...
	m := new(starkcurve.G1Jac).ScalarMultiplication(p1, lowPart.ToBigInt(new(big.Int)))

	highPart := new(big.Int).SetUint64(a.ToRegular()[3] >> lowPartBits & 0xf) // The top nibble (bits 249-252)
	n := new(starkcurve.G1Jac).ScalarMultiplication(p2, highPart)
